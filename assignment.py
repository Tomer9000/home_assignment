...
parser = argparse.ArgumentParser(
    description='This script lists all the files in a directory recursively.')
...
def calc_md5(file):
    ...
    # If the file was not found.
        ...
        print(f"File {e.filename} was not found, "
              f"probably moved/deleted during the running of the script.")
    ...
    try:
        with open(CSV_NAME, mode='w') as csv_file:
            ...
            for file_path, md5_file in dict_files.items():
                writer.writerow({'number': number, 'file-path': file_path,
                                 'md5-of-the-file': md5_file})
                number += 1
    # If there are no permissions to create the csv file.
    except PermissionError as e:
        print(f"{getpass.getuser()} has no permissions "
              f"to create csv file: {e.filename}")
